-- =====================================================================
-- PROVODKA — AYLANMA Q2a («bizdan qarzdor») TUZATISHI (2026-09-09)
-- ---------------------------------------------------------------------
-- ## MUAMMO (Asilbek: «qarzni yana noto'g'ri ko'rsatyapti, faqat hamyon
--    balansni ko'rsatyaptimi deyman»)
--
--   Q2a eski kodi (PROVODKA_AYLANMA.sql 812-865):
--       v_num  = Provodka qarz            (bizdagi jadval)
--       v_num2 = summary ->> 'total_debt' (Aros yig'indisi)  <-- YAGONA manba
--       v_bola_uzs = sum(aros_qarzdor.wallet_balance)        (BIZDAGI jadval)
--       Q2a = v_num + v_num2 - v_bola_uzs
--
--   `summary` null bo'lsa, `total_debt` kaliti bo'lmasa yoki
--   `aros_qarzdor_sync` da id=1 qatori bo'lmasa -> `coalesce(..., 0)` tufayli
--   Aros qarzi JIMGINA 0 bo'lardi. Hamyon esa `aros_qarzdor` JADVALIDAN
--   olingani uchun BARIBIR ayirilardi (jadvalda qatorlar bor). Natija:
--
--       Q2a = provodka + 0 - hamyon      -> ekranda «minus hamyon» ko'rinardi
--
--   Aynan shu — «faqat hamyon balansni ko'rsatyapti». Bundan tashqari bu
--   PROVODKA_AYLANMA.sql ning O'Z qoidasini buzardi:
--       «manba yo'q -> bo'lim null + toliq=false + xatolar[]; JIMGINA 0 YO'Q».
--
--   Ikkinchi kamchilik: qarz `summary` dan (Aros qamrovi), hamyon esa
--   jadvaldan (biz sinxronlagan qamrov) olinardi — IKKI XIL BAZA aralashardi.
--
-- ## YECHIM
--   Qarz va hamyon endi AYNI manbadan olinadi:
--     1) `summary` bor bo'lsa : qarz = summary.total_debt,
--                               hamyon = summary.total_wallet_balance
--     2) bo'lmasa jadvaldan   : qarz = sum(total_debt) faol,
--                               hamyon = sum(wallet_balance) faol
--     3) ikkalasi ham yo'q    : Q2a = null + toliq=false + xatolar[]  (0 EMAS)
--   Ishlatilgan manba `meta.manba` ('summary'|'jadval') da qoladi —
--   drill-down'da ko'rinadi, keyingi safar taxmin qilish shart emas.
--
-- ## 🔴 RUN QILGANDAN KEYIN
--   Bu funksiya faqat YANGI snapshotga ta'sir qiladi. Ekrandagi eski raqam
--   o'zgarishi uchun Aylanma sahifasida «Hisoblash» tugmasini bosing
--   (yoki n8n `o3BZP8uYatGkRu8b` ni bir marta ishga tushiring).
--
-- ## QOIDALAR (CLAUDE.md)
--   * ADDITIVE: imzo `sync_aylanma_snapshot(jsonb)` O'ZGARMAYDI.
--   * Funksiya tanasi PROVODKA_AYLANMA.sql (318-1053) dan VERBATIM ko'chirildi,
--     FAQAT Q2a bloki almashtirildi. Boshqa bo'limlarga (A,B,T1,T5,Y3a,K3b,K4,
--     B6,Q2b,D) bironta qator ham tegilmagan.
...
begin
  ...
  if to_regclass('public.aros_qarzdor') is null and to_regclass('public.aros_qarzdor_sync') is null then
    raise exception 'aros_qarzdor / aros_qarzdor_sync yoq — avval PROVODKA_AROS_QARZDOR.sql ni bajaring';
  end if;
end
$q2afix_pre$;
...
-- #####################################################################
-- ##  1-BO'LIM — sync_aylanma_snapshot(jsonb) — Q2a bloki tuzatildi   ##
-- #####################################################################
...
comment on function sync_aylanma_snapshot(jsonb) is
  ...
  '(summary, bo''lmasa jadval); manba umuman yo''q bo''lsa 0 emas, XATO beradi.';
...
-- #####################################################################
-- ##  2-BO'LIM — TEKSHIRUV (faqat select)                             ##
-- #####################################################################

-- 2.1 Manba bormi va nima deyapti. `summary_total_debt` NULL bo'lsa — eski
--     kod aynan shu sababdan 0 qo'yib, hamyonni ayirib yuborardi.
select
  (select count(*) from aros_qarzdor_sync)                              as sync_qator,
  (select s.summary is not null from aros_qarzdor_sync s where s.id=1)  as summary_bor,
  (select (s.summary ->> 'total_debt')::numeric
     from aros_qarzdor_sync s where s.id=1)                             as summary_total_debt,
  (select (s.summary ->> 'total_wallet_balance')::numeric
     from aros_qarzdor_sync s where s.id=1)                             as summary_hamyon,
  (select s.synced_at from aros_qarzdor_sync s where s.id=1)            as synced_at,
  (select coalesce(sum(a.total_debt),0)     from aros_qarzdor a where a.faol) as jadval_qarz,
  (select coalesce(sum(a.wallet_balance),0) from aros_qarzdor a where a.faol) as jadval_hamyon,
  (select count(*) from aros_qarzdor a where a.faol)                    as jadval_faol_soni;

-- 2.2 `summary` ning HAMMA kaliti — kalit nomi kutilganidan boshqa bo'lsa shu yerda ko'rinadi
select k.key, left(k.value #>> '{}', 40) as qiymat
  from aros_qarzdor_sync s, jsonb_each(s.summary) k
 where s.id = 1
 order by k.key;

-- 2.3 Q2a endi qanday chiqadi (funksiyani ishga tushirmasdan, ayni formula bilan)
with src as (
  select case when (select (s.summary ->> 'total_debt')::numeric from aros_qarzdor_sync s where s.id=1) is not null
              then 'summary' else 'jadval' end as manba
),
v as (
  select (select coalesce(sum(h.qolgan),0)
            from qarz q join v_qarz_holat h on h.qarz_id=q.id
           where q.status='faol') as provodka,
         case when (select manba from src)='summary'
              then (select (s.summary ->> 'total_debt')::numeric from aros_qarzdor_sync s where s.id=1)
              else (select coalesce(sum(a.total_debt),0) from aros_qarzdor a where a.faol) end as qarz,
         case when (select manba from src)='summary'
              then coalesce((select (s.summary ->> 'total_wallet_balance')::numeric from aros_qarzdor_sync s where s.id=1),0)
              else (select coalesce(sum(a.wallet_balance),0) from aros_qarzdor a where a.faol) end as hamyon
)
select (select manba from src) as manba, provodka, qarz, hamyon,
       provodka + qarz - hamyon as q2a_yangi,
       provodka + qarz          as q2a_hamyonsiz
  from v;

-- 2.4 Oxirgi snapshotdagi ESKI Q2a (taqqoslash uchun; «Hisoblash» bosilgach yangilanadi)
select q.ref, q.nom, q.uzs, q.meta
  from aylanma_qator q
 where q.snapshot_id = (select id from aylanma_snapshot order by sana desc, hisoblangan_at desc limit 1)
   and q.bolim = 'Q2a'
 order by q.ref;

-- 🔴 ESLATMA: 2.3 dagi `q2a_yangi` — «Hisoblash» bosilgandan keyin chiqadigan raqam.
--    Agar u ham kutganingizdan farq qilsa, hamyon ayirilishining O'ZI noto'g'ri
--    degani (2026-09-08 qarori) — o'shanda `q2a_hamyonsiz` ni tanlaymiz.
