-- =====================================================================
--  PROVODKA_JURNAL_MAYDON.sql — Jurnal: XARAJAT MODDASI MAXSUS MAYDONI
--  (entry_maydon.element_id) bo'yicha SERVER TOMONDA filtr ("Tag")
-- ---------------------------------------------------------------------
--  Project: Provodka (kxzerccdpcltmzrxutlo).  TaskFix EMAS.
--  Asilbek qarori: xarajat turi (modda) uchun "royxat" turidagi maxsus
--  maydon bo'lsa (masalan "Gaz" moddasiga "Mashina" maydoni biriktirilgan,
--  variantlari "Tracker" / "Damas"), jurnalda shu variant bo'yicha ham
--  filtrlash kerak — SERVER tomonda (sanoq, sahifalash, Excel to'g'ri
--  chiqsin, klient filtri buni ta'minlay olmaydi).
--
--  NIMA QILADI
--  -----------
--  To'rtala jurnal RPC siga OXIRGI argument qilib `p_elementlar uuid[]
--  default null` qo'shiladi: `jurnal_v2_baza`, `jurnal_v2`, `jurnal_v2_count`,
--  `jurnal_dash`. Bo'sh/null bo'lsa xatti-harakat AYNAN eskidek. Aks holda
--  faqat entry_maydon jadvalida (`PROVODKA_XARAJAT_MAYDON.sql`) KAMIDA BITTA
--  qatori `element_id = any(p_elementlar)` bo'lgan yozuvlar qaytadi
--  (`exists`, `entry_maydon_element_idx` indeksi ishlaydi).
--
--  🔴 PROD TA'SIR YO'Q: `jurnal.html` (va `jurnal-dev.html` ning eski kodi)
--  `sb.rpc('jurnal_v2', {...})` ni **nomlangan argumentlar** bilan chaqiradi —
--  `p_elementlar` kalitini yubormaydi, funksiya sukut (`null`) bilan
--  ishlaydi va natija bitta bit ham o'zgarmaydi. Xuddi shu sabab bilan
--  `jurnal_ijrochilar`/`jurnal_qoldiq` (bu faylda TEGILMAGAN) ham
--  `jurnal_v2_baza`ni eski 6-argumentli chaqiruv bilan ishlatishda davom
--  etadi — 7-parametr default'ga tushadi.
--
--  🔴 MANBA — ENG OXIRGI TANA QAYERDAN: jurnal_v2_baza/jurnal_v2 zanjiri
--  PROVODKA_IJROCHI.sql dan keyin YANA IKKI marta ustidan yozilgan:
--    PROVODKA_JURNAL_KIRITILGAN.sql (sana filtri entry_date -> created_at)
--    -> PROVODKA_JURNAL_PUL.sql ('pul' tokeni)
--    -> PROVODKA_KONVERT_V3.sql ('savdosiz' tokeni, repoda .sql sifatida yo'q)
--    -> PROVODKA_KONVERT_FILTR.sql ('konvert' tokeni + jurnal_v2'ga ext_ref).
--  Shuning uchun bu faylda tana **PROVODKA_KONVERT_FILTR.sql** dagi (ENG
--  OXIRGI, hammasini o'z ichiga olgan) versiyadan AYNAN ko'chirildi — undan
--  eskisidan (masalan PROVODKA_IJROCHI.sql) olinsa 'pul'/'savdosiz'/
--  'konvert' tokenlari va ext_ref kaliti yo'qolib, prod jurnal.html hozir
--  ishlatayotgan filtrlar SINARDI (baza bitta — hamma jurnal RPC shu bilan).
--  `jurnal_v2_count`/`jurnal_dash` esa PROVODKA_IJROCHI.sql dan keyin qayta
--  yozilmagan (KIRITILGAN/KONVERT_FILTR buni o'z DIAG blokida tasdiqlaydi) —
--  ular uchun o'sha faylning tanasi AYNAN.
--
--  SQL ADDITIVE: jadval/ustun o'chirilmaydi. Funksiya IMZOSI o'zgargani
--  uchun (yangi parametr) har birining ustida `drop function if exists`
--  turadi — CREATE bilan AJRALMAS, aks holda PostgREST eski+yangi
--  overload'ni ikkalasini ham ko'rib "PGRST203 could not choose the best
--  candidate" berishi mumkin (PROVODKA_JURNAL_KIRITILGAN.sql dagi saboq).
--
--  OLD SHART: PROVODKA_XARAJAT_MAYDON.sql (entry_maydon jadvali) va
--  PROVODKA_KONVERT_FILTR.sql (jurnal_konvert_filtr_ok — hozirgi eng oxirgi
--  jurnal_v2_baza/jurnal_v2 zanjiri) RUN qilingan bo'lsin.
-- =====================================================================
...
do $do$
begin
  if to_regclass('public.entry_maydon') is null then
    raise exception
      'Old shart yoq: entry_maydon jadvali topilmadi (PROVODKA_XARAJAT_MAYDON.sql avval RUN qilinsin).'
      using errcode = '55000';
  end if;
  if to_regprocedure('public.jurnal_v2_baza(date,date,uuid[],uuid[],text[],text)') is null
     or to_regprocedure('public.jurnal_konvert_filtr_ok()') is null then
    raise exception
      'Old shart yoq: PROVODKA_KONVERT_FILTR.sql avval RUN qilingan bolishi kerak '
      '(jurnal_v2_baza(date,date,uuid[],uuid[],text[],text) va jurnal_konvert_filtr_ok() '
      'topilmadi). Bu faylni RUN qilish hozircha bekor qilindi.'
      using errcode = '55000';
  end if;
  if to_regprocedure('public.jurnal_v2_count(date,date,uuid[],uuid[],text[],text,text)') is null
     or to_regprocedure('public.jurnal_dash(date,date,uuid[],uuid[],text[],text,text)') is null then
    raise exception
      'Old shart yoq: jurnal_v2_count/jurnal_dash (7-argumentli, PROVODKA_IJROCHI.sql) topilmadi.'
      using errcode = '55000';
  end if;
end
$do$;
  ...
  with e as (
    select en.id                as e_id,
     ...
     where en.status = 'posted'
       ...
       and (v_elementlar is null or exists (
...
revoke all on function jurnal_v2_baza(date, date, uuid[], uuid[], text[], text, uuid[]) from public, anon, authenticated;

comment on function jurnal_v2_baza(date, date, uuid[], uuid[], text[], text, uuid[]) is
      ...
      select p.*,
        ...
        from (
          ...
          select b.id, b.entry_date, b.description, b.source,
            ...
            from jurnal_v2_baza(p_from, p_to, p_accounts, p_moddalar, p_turlar, p_q, p_elementlar) b
...
revoke all on function jurnal_v2(date, date, uuid[], uuid[], text[], text, int, int, text, uuid[]) from public, anon;
grant execute on function jurnal_v2(date, date, uuid[], uuid[], text[], text, int, int, text, uuid[]) to authenticated;

comment on function jurnal_v2(date, date, uuid[], uuid[], text[], text, int, int, text, uuid[]) is
...
revoke all on function jurnal_v2_count(date, date, uuid[], uuid[], text[], text, text, uuid[]) from public, anon;
grant execute on function jurnal_v2_count(date, date, uuid[], uuid[], text[], text, text, uuid[]) to authenticated;

comment on function jurnal_v2_count(date, date, uuid[], uuid[], text[], text, text, uuid[]) is
...
begin
  ...
  with b as materialized (
    ...
    select * from jurnal_v2_baza(p_from, p_to, p_accounts, p_moddalar, null, p_q, p_elementlar) z
     where v_ij is null
...
revoke all on function jurnal_dash(date, date, uuid[], uuid[], text[], text, text, uuid[]) from public, anon;
grant execute on function jurnal_dash(date, date, uuid[], uuid[], text[], text, text, uuid[]) to authenticated;

comment on function jurnal_dash(date, date, uuid[], uuid[], text[], text, text, uuid[]) is
...
-- #####################################################################
-- ##  5. jurnal_maydon_filtr_ok() — klient uchun "SQL RUN qilingan"   ##
-- ##     belgisi (jurnal_pul_filtr_ok()/jurnal_savdosiz_ok() naqshi)  ##
-- #####################################################################
create or replace function jurnal_maydon_filtr_ok()
returns boolean
language sql
stable
as $fn$ select true $fn$;

revoke all on function jurnal_maydon_filtr_ok() from public, anon;
grant execute on function jurnal_maydon_filtr_ok() to authenticated;

comment on function jurnal_maydon_filtr_ok() is
  'jurnal-dev.html: jurnal_v2/jurnal_v2_count/jurnal_dash p_elementlar (xarajat maydon TAG filtri) '
  'argumentini tushunadimi (PGRST202 bolsa yoq — eski RPC yubormaydi, Tag chipi yashirin qoladi).';
...
-- #####################################################################
-- ##  DIAG — FAQAT KATALOG SO'ROVLARI (jonli RPC chaqiruvi yo'q)      ##
-- #####################################################################
-- 🔴 `pg_get_function_identity_arguments` ATAYLAB ISHLATILMAYDI (parametr
--    nomini ham qaytaradi, matn taqqoslashni chalkashtiradi) — buning
--    o'rniga `oidvectortypes(proargtypes)` (faqat tur ro'yxati) ishlatiladi.
...
begin
  ...
  raise notice 'jurnal_v2_baza turlari: %', coalesce(v_baza, '(topilmadi)');
  raise notice 'jurnal_v2 turlari: %', coalesce(v_v2, '(topilmadi)');
  raise notice 'jurnal_v2_count turlari: %', coalesce(v_count, '(topilmadi)');
  raise notice 'jurnal_dash turlari: %', coalesce(v_dash, '(topilmadi)');

  if v_baza = 'date, date, uuid[], uuid[], text[], text, uuid[]' then
    raise notice 'jurnal_v2_baza: YANGI imzo (p_elementlar) OK';
  else
    raise notice 'jurnal_v2_baza: KUTILMAGAN imzo — tekshiring';
  end if;

  if v_v2 = 'date, date, uuid[], uuid[], text[], text, integer, integer, text, uuid[]' then
    raise notice 'jurnal_v2: YANGI imzo (p_elementlar) OK';
  else
    raise notice 'jurnal_v2: KUTILMAGAN imzo — tekshiring';
  end if;

  if v_count = 'date, date, uuid[], uuid[], text[], text, text, uuid[]' then
    raise notice 'jurnal_v2_count: YANGI imzo (p_elementlar) OK';
  else
    raise notice 'jurnal_v2_count: KUTILMAGAN imzo — tekshiring';
  end if;

  if v_dash = 'date, date, uuid[], uuid[], text[], text, text, uuid[]' then
    raise notice 'jurnal_dash: YANGI imzo (p_elementlar) OK';
  else
    raise notice 'jurnal_dash: KUTILMAGAN imzo — tekshiring';
  end if;

  -- Overload bittadan — eski (p_elementlar'siz) imzo qolmagan bo'lishi shart.
  if (select count(*)::int from pg_proc p where p.pronamespace = 'public'::regnamespace and p.proname = 'jurnal_v2_baza') <> 1 then
    raise notice 'jurnal_v2_baza: OVERLOAD BOR — eski imzo qolib ketgan bolishi mumkin!';
  end if;
  if (select count(*)::int from pg_proc p where p.pronamespace = 'public'::regnamespace and p.proname = 'jurnal_v2') <> 1 then
    raise notice 'jurnal_v2: OVERLOAD BOR — eski imzo qolib ketgan bolishi mumkin!';
  end if;
  if (select count(*)::int from pg_proc p where p.pronamespace = 'public'::regnamespace and p.proname = 'jurnal_v2_count') <> 1 then
    raise notice 'jurnal_v2_count: OVERLOAD BOR — eski imzo qolib ketgan bolishi mumkin!';
  end if;
  if (select count(*)::int from pg_proc p where p.pronamespace = 'public'::regnamespace and p.proname = 'jurnal_dash') <> 1 then
    raise notice 'jurnal_dash: OVERLOAD BOR — eski imzo qolib ketgan bolishi mumkin!';
  end if;
end
$diag$;

select to_regprocedure('public.jurnal_maydon_filtr_ok()') is not null as probe_bor,
       to_regclass('public.entry_maydon')                 is not null as entry_maydon_bor,
       to_regclass('public.entry_maydon_element_idx')      is not null as element_idx_bor;


-- #####################################################################
-- ##  ROLLBACK (qo'lda, kerak bo'lsa)                                 ##
-- #####################################################################
-- 1) jurnal_v2_baza/jurnal_v2 — PROVODKA_KONVERT_FILTR.sql qayta RUN qilinsa
--    tana p_elementlar'siz (6/9 argumentli) qaytadi. Lekin 42P13/overload
--    tuzog'iga tushmaslik uchun avval SHU faylning drop qatorlarini emas,
--    balki quyidagi drop'larni qo'lda bajaring, keyin KONVERT_FILTR.sql'ni RUN qiling:
-- drop function if exists jurnal_v2_baza(date, date, uuid[], uuid[], text[], text, uuid[]);
-- drop function if exists jurnal_v2(date, date, uuid[], uuid[], text[], text, int, int, text, uuid[]);
-- 2) jurnal_v2_count/jurnal_dash — PROVODKA_IJROCHI.sql 3.3/3.4 bo'limini qayta RUN qiling:
-- drop function if exists jurnal_v2_count(date, date, uuid[], uuid[], text[], text, text, uuid[]);
-- drop function if exists jurnal_dash(date, date, uuid[], uuid[], text[], text, text, uuid[]);
-- 3) drop function if exists jurnal_maydon_filtr_ok();
